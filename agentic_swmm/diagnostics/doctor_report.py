...
This module is a pure data layer — literally, since the ``--fix``
machinery moved to ``agentic_swmm.diagnostics.fixes``: collectors and
``*_to_dict`` serializers return data, render functions return strings
(no IO), and the dispatcher in ``commands/doctor.py`` prints them.
"""
