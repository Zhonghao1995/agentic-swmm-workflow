...
The case-defaults store (``gap_defaults.yaml`` schema + reader/writer)
lives in ``agentic_swmm/case/case_defaults.py`` — this module is the
thin CLI verb over it. (The original PRD kept the store self-contained
in this file; the 2026-07 architecture pass overturned that so the
store is discoverable from the library layer like every other store.)
"""
