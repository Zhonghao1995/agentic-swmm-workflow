...
def _build_setup_state(*, config_file: Path, skills_file: Path, mcp_file: Path, memory_file: Path, provider: str, model: str | None) -> dict:
    ...
    checks = [
        ...
        _check("node executable", shutil.which("node") is not None, shutil.which("node") or "missing; needed for MCP launchers", required=True),
        _check("swmm5 executable", _which_swmm5() is not None, _which_swmm5() or "missing; needed for SWMM execution", required=True),
