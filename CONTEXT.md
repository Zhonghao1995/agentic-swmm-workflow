...
The registry (`AgentToolRegistry` in `tool_registry.py`) is the single source of truth for which tools the planner can see. `compute_intent_signals` also lives here as a thin legacy-adapter for the warm-intro classifier (see `tool_registry.py` split work in PR #128).
...
4. **Modeling memory only mutates via explicit verbs.** No agent action writes to `memory/modeling-memory/` autonomously. Explicit mutation verbs include: `aiswmm gap promote-to-case`, `aiswmm cite`, `aiswmm calibration accept`, `aiswmm memory archive`, `aiswmm memory restore`. Read-time health-tier filtering (PR 4) is NOT a mutation — the live store is unchanged until the user explicitly runs `archive`. (Auto-distillation is an open research direction — see private PRD-12.)
...
**Why swmm-gis stays unregistered.** Six of the eleven tools (`qgis_load_layers`, `qgis_validate_crs`, `qgis_normalize_layers`, `qgis_overlay_landuse_soil`, `qgis_import_drainage_assets`, `qgis_export_swmm_intermediates`) invoke `qgis_prepare_swmm_inputs.py`, which shells out to QGIS Processing (hardcoded default: `QGIS-final-4_0_2.app`) and optionally GRASS (`GRASS-8.4.app`). Three more (`qgis_package_final_layers`, `qgis_area_weighted_params`, `basin_shp_to_subcatchments`) need the heavy geo stack (`geopandas`, plus `rasterio`/`numpy` for packaging) though not the QGIS desktop apps. Registering typed ToolSpecs for tools that require a desktop GIS installation or optional heavy dependencies would add significant schema surface to the LLM's flat tool registry for tools that fail in a standard Python environment. Only two tools are pure-stdlib — `gis_preprocess_subcatchments` and `qgis_extract_slope_area_width`, both backed by `preprocess_subcatchments.py` (the `qgis_` prefix on the latter is historical; it does not touch QGIS) — and those two are **hot-path candidates for typed promotion** if agent flows start using them frequently; see the promotion path below.
...
**Promotion path.** If `gis_preprocess_subcatchments` or `qgis_extract_slope_area_width` (pure-stdlib) — or `basin_shp_to_subcatchments` (needs `geopandas` only) — appear frequently in `agent_trace.jsonl` as `call_mcp_tool` calls, promote them to typed ToolSpecs in a follow-up PR; their schemas are already defined in `mcp/swmm-gis/server.js` and none of them requires the QGIS desktop apps.
